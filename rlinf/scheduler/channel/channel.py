...
class Channel:
    ...
    **Features**:
    ...
    @staticmethod
    def _put_many_async_channel_key(items: list[tuple[Any, Any, int]]) -> str:
        # AsyncChannelWork serializes by channel key. Use the target key set so
        # one manager's split slots stay ordered while disjoint managers can run concurrently.
        key_parts = sorted({str(key) for key, _item, _weight in items})
        return "__put_many__:" + "|".join(key_parts)
    ...
    def put_many(
        ...
        """Put multiple keyed items with one channel operation.

        Each tuple is ``(key, item, weight)``. Items are inserted in the
        provided order, while each key still has its own FIFO queue.
        """
        if self._local_channel is not None:
            assert async_op is False, "Local channel does not support async put."
            self._local_channel.put_many(items)
            return
        ...
        if self._current_worker is not None:
            ...
            async_channel_work = AsyncChannelWork(
                channel_name=self._channel_name,
                channel_key=self._put_many_async_channel_key(items),
                channel_actor=self._channel_worker_actor,
                method="put_many",
                **put_kwargs,
            )
            self._current_worker.send(
                items, self._channel_name, 0, async_op=True
            )
            ...
            put_kwargs = {"items": items}
            async_channel_work = AsyncChannelWork(
                channel_name=self._channel_name,
                channel_key=self._put_many_async_channel_key(items),
                channel_actor=self._channel_worker_actor,
                method="put_many_via_ray",
                **put_kwargs,
            )
            if async_op:
                return async_channel_work
    ...
    def get_any(
        ...
        keys: list[Any],
        async_op: bool = False,
    ) -> AsyncWork | tuple[Any, Any]:
        """Get an item from the first available queue among ``keys``."""
        if self._local_channel is not None:
            assert async_op is False, "Local channel does not support async get."
            return self._local_channel.get_any(keys)
        ...
        if self._current_worker is not None:
            ...
            get_kwargs = {"keys": keys}
            async_channel_work = AsyncChannelWork(
                channel_name=self._channel_name,
                channel_key="__get_any__",
                channel_actor=self._channel_worker_actor,
                method="get_any_via_ray",
                **get_kwargs,
            )
            if async_op:
                return async_channel_work
    ...
    def get_any_timeout(self, keys: list[Any], timeout_s: float) -> tuple[Any, Any]:
        """Get an item from the first available queue, waiting at most ``timeout_s`` seconds."""
        timeout_s = 0.0 if timeout_s is None else max(0.0, float(timeout_s))
        if self._local_channel is not None:
            return self._local_channel.get_any(keys, timeout_s=timeout_s)
        ...
        get_kwargs = {"keys": keys, "nowait": False, "timeout_s": timeout_s}
        async_channel_work = AsyncChannelWork(
            channel_name=self._channel_name,
            channel_key="__get_any__",
            channel_actor=self._channel_worker_actor,
    ...
    def get_any_nowait(self, keys: list[Any]) -> tuple[Any, Any]:
        """Get an item from the first available queue among ``keys`` without waiting."""
        if self._local_channel is not None:
            return self._local_channel.get_any(keys, nowait=True)
        ...
        if self._current_worker is not None:
            ...
            get_kwargs = {"keys": keys, "nowait": True}
            async_channel_work = AsyncChannelWork(
                channel_name=self._channel_name,
                channel_key="__get_any__",
                channel_actor=self._channel_worker_actor,
