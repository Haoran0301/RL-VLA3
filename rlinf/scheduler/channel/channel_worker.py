...
class LocalChannel:
    ...
    def put_many(
        ...
        """Put multiple keyed items into local queues in order."""
        for key, item, weight in items:
            self.put(item=item, weight=weight, key=key, nowait=nowait)
    ...
    def get_any(
        ...
        while True:
            for i in range(key_count):
                key = keys[(rr + i) % key_count]
                self.create_queue(key, maxsize=self.maxsize())
                if self._queue_map[key].empty():
                    continue
                weighted_item: WeightedItem = self._queue_map[key].get_nowait()
                return key, weighted_item.item
            if nowait or (deadline is not None and time.time() >= deadline):
                raise asyncio.QueueEmpty
            rr = (rr + 1) % key_count
            sleep_s = 0.001
...
class ChannelWorker(Worker):
    ...
    async def put_many(
        ...
        """Put multiple keyed items received from a worker."""
        items = self.recv(src_addr.root_group_name, src_addr.rank_path)
        for key, item, weight in items:
            self.create_queue(key, self.maxsize())
    ...
    async def put_many_via_ray(
        self,
        items: list[tuple[Any, Any, int]],
        nowait: bool = False,
    ):
        """Put multiple keyed items via Ray communication."""
        for key, item, weight in items:
            self.create_queue(key, self.maxsize())
            weighted_item = WeightedItem(weight=weight, item=item)
            if nowait:
    ...
    async def get_any(
        ...
        rr = 0
        result_key = None
        weighted_item: WeightedItem | None = None
        while weighted_item is None:
            for i in range(key_count):
                key = keys[(rr + i) % key_count]
                try:
                    weighted_item = self._queue_map[key].get_nowait()
                    result_key = key
                    break
                except asyncio.QueueEmpty:
                    continue
            if weighted_item is not None:
                break
            if nowait or (deadline is not None and loop.time() >= deadline):
                query_id = asyncio.QueueEmpty
                result_key = None
                weighted_item = WeightedItem(weight=0, item=None)
                break
        ...
        self.send(
            (query_id, (result_key, weighted_item.item)),
            dst_addr.root_group_name,
            dst_addr.rank_path,
            async_op=True,
        )
    ...
    async def get_any_via_ray(
        self,
        keys: list[Any],
        nowait: bool = False,
        timeout_s: float | None = None,
    ) -> tuple[Any, Any]:
        """Get an item from the first available queue among ``keys`` via Ray."""
        if not keys:
            raise asyncio.QueueEmpty
