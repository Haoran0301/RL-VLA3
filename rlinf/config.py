...
def validate_cfg(cfg: DictConfig) -> DictConfig:
    ...
    if cfg.actor.training_backend == "megatron":
        ...
        if cfg.runner.task_type == "embodied":
            assert cfg.actor.global_batch_size % actor_world_size == 0, (
                f"actor.global_batch_size ({cfg.actor.global_batch_size}) must be "
                f"divisible by actor_world_size ({actor_world_size})"
            )
        else:
            assert (
                cfg.actor.global_batch_size
                ...
                f"actor.global_batch_size ({cfg.actor.global_batch_size}) must be "
                f"divisible by (actor.micro_batch_size "
