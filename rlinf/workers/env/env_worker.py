...
class EnvWorker(Worker):
    def __init__(self, cfg: DictConfig):
        ...
        # Support both cases:
        # 1. rollout >= env: each env serves multiple rollouts (gather_num >= 1)
        # 2. env > rollout: multiple envs serve one rollout (gather_num < 1, use scatter_num)
        if rollout_world_size >= env_world_size:
            assert rollout_world_size % env_world_size == 0, (
                f"rollout_world_size ({rollout_world_size}) must be divisible by "
                f"env_world_size ({env_world_size})"
            )
            # gather_num: number of rollout for each env process
            self.gather_num = rollout_world_size // env_world_size
            self.scatter_num = 1  # each env sends to gather_num rollouts
        else:
            assert env_world_size % rollout_world_size == 0, (
                f"env_world_size ({env_world_size}) must be divisible by "
                f"rollout_world_size ({rollout_world_size})"
            )
            # scatter_num: number of env processes per rollout
            self.scatter_num = env_world_size // rollout_world_size
            self.gather_num = 1  # each env sends to 1 rollout
    ...
    def recv_chunk_actions(self, input_channel: Channel, mode="train") -> np.ndarray:
        ...
        if self.scatter_num == 1:
            # Original case: each env serves multiple rollouts
            ...
            # New case: multiple envs serve one rollout
            # Each env receives its portion from the rollout
            rollout_id = self._rank // self.scatter_num
            env_local_id = self._rank % self.scatter_num
            chunk_action = input_channel.get(key=f"{rollout_id}_{env_local_id}_{mode}")
        return chunk_action
    ...
    def send_env_batch(self, output_channel: Channel, env_batch, mode="train"):
        ...
        if self.scatter_num == 1:
            ...
            # New case: multiple envs serve one rollout
            # Each env sends to the rollout it's assigned to with a sub-key
            rollout_id = self._rank // self.scatter_num
            env_local_id = self._rank % self.scatter_num
            output_channel.put(
                item=env_batch,
                key=f"{rollout_id}_{env_local_id}_{mode}",
            )
