...
def _select_mujoco_egl_device(seed_offset, worker_info):
    candidate_devices = []
    if worker_info is not None:
        candidate_devices.append(getattr(worker_info, "accelerator_rank", None))
    candidate_devices.append(os.environ.get("MUJOCO_EGL_DEVICE_ID"))
    candidate_devices.append(os.environ.get("CUDA_VISIBLE_DEVICES", "").split(",")[0])
    ...
    return seed_offset % 8


def _log_mujoco_egl_device(seed_offset, worker_info, egl_device_id):
    # region agent log
    try:
        with open("/ufs/.cursor/debug-2977cf.log", "a", encoding="utf-8") as f:
            f.write(
                json.dumps(
                    {
                        "sessionId": "2977cf",
                        "runId": "post-fix",
                        "hypothesisId": "H1",
                        "location": "rlinf/envs/metaworld/metaworld_env.py:_log_mujoco_egl_device",
                        "message": "Selected MuJoCo EGL device for MetaWorld env worker",
                        "data": {
                            "seed_offset": seed_offset,
                            "worker_accelerator_rank": getattr(
                                worker_info, "accelerator_rank", None
                            )
                            if worker_info is not None
                            else None,
...
class MetaWorldEnv(gym.Env):
    ...
    def get_env_fns(self):
        ...
        env_fns = []
        for local_env_idx, env_fn_param in enumerate(env_fn_params):

            def env_fn(param=env_fn_param, should_log_egl_device=local_env_idx == 0):
                egl_device_id = _select_mujoco_egl_device(
                    self.seed_offset, self.worker_info
                )
                os.environ["MUJOCO_EGL_DEVICE_ID"] = str(egl_device_id)
                if should_log_egl_device:
                    _log_mujoco_egl_device(
                        self.seed_offset, self.worker_info, egl_device_id
                    )
